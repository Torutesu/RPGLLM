...
async function buildAWorld(page: Page, request: APIRequestContext, account: Account): Promise<StudioWorld> {
  ...
  await expect(page.getByTestId(T.studioBuilding), "SCR-049 must take over from the CTA").toBeVisible({
    timeout: 15_000,
  });

  await buildWorlds(request);
  await expect(page.getByTestId(T.studioReady), "the build must finish and reveal the world").toBeVisible({
    timeout: 30_000,
  });
...
test.describe("World Studio", () => {
  ...
  test("E2E-029: a world built from one line is really playable", async ({ page, request }) => {
    ...
    expect((await wallet(request, account.jwt)).gems, "a new account can afford exactly one world").toBe(
      WORLD_STUDIO.STARTER_GEMS,
    );
    ...
    await expect(cast.locator('[data-testid^="studio-cast-"]')).toHaveCount(WORLD_STUDIO.CAST_SIZE, {
      timeout: 10_000,
    });

    expect((await wallet(request, account.jwt)).gems, "the build cost exactly one world").toBe(
      WORLD_STUDIO.STARTER_GEMS - WORLD_STUDIO.GEM_COST,
    );
    ...
    await expect(page.getByTestId(T.feedList), "the world someone typed must produce a live feed").toBeVisible({
      timeout: 15_000,
    });
    await expect(page.locator('[data-testid^="post-"]').first(), "the feed must not be empty").toBeVisible({
      timeout: 15_000,
    });
  });
  ...
  test("E2E-030: a refused premise costs nothing", async ({ page, request }) => {
    ...
    await expect(page.getByTestId(T.studioError), "the refusal must be said out loud on SCR-048").toBeVisible({
      timeout: 15_000,
    });
  ...
  test("E2E-031: `everyone` waits for a person", async ({ page, request }) => {
    ...
    await expect(page.getByTestId(T.studioStatusBadge), "the world must say it is being read").toBeVisible({
      timeout: 15_000,
    });
    ...
    expect(
      shelf.map((w) => w.id),
      "an approved world reaches Explore",
    ).toContain(world.id);
    ...
    await expect(
      page.getByTestId(T.communityWorldCard(world.slug)),
      "Explore must show the approved world",
  ...
  test("E2E-032: someone else's private world does not exist", async ({ page, request }) => {
    ...
    await expect(page.getByTestId(T.worldCard(world.slug)), "someone else's world is not in the picker").toHaveCount(0);
  });
  ...
  test("E2E-033: a build that dies gives the gems back", async ({ page, request }) => {
    ...
    expect((await wallet(request, account.jwt)).gems, "the charge lands before the build runs").toBe(
      before - WORLD_STUDIO.GEM_COST,
    );
    ...
    await expect(page.getByTestId(T.studioFailed), "a dead build must say so, not spin forever").toBeVisible({
      timeout: 30_000,
    });
  ...
  test("E2E-034: enough reporters take a live world off the shelf", async ({ page, request }) => {
    ...
    expect(
      (await publicWorlds(request, watcher.jwt)).map((w) => w.id),
      "an approved world is on the shelf",
    ).toContain(world.id);
    ...
    expect(
      (await publicWorlds(request, watcher.jwt)).map((w) => w.id),
      "a world under the threshold stays up",
    ).toContain(world.id);
    ...
    expect(
      (await publicWorlds(request, watcher.jwt)).map((w) => w.id),
      "the reported world leaves Explore",
  ...
  test("E2E-035: a turned-down world cannot be bounced straight back at the queue", async ({ page, request }) => {
    ...
    await call(request, author.jwt, "POST", `/v1/admin/worlds/${world.id}/review`, {
      decision: "reject",
      reason: "Reads as an existing show with the names changed.",
    });

    await gotoApp(page, `/studio/${world.id}`);
    // The creator is told what was wrong, in the reviewer's own words.
    await expect(page.getByText("Reads as an existing show with the names changed.")).toBeVisible({ timeout: 15_000 });
    ...
    await publish.click();
    await expect(page.getByText(strings.en.studioResubmitWait), "a refused resubmit says why").toBeVisible({
      timeout: 15_000,
    });
